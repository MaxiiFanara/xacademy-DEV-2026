...
-- Para el listado de cards del front (pantalla principal).
-- Muestra solo la posición principal (EsPrincipal = TRUE)
-- de cada jugador por versión, sin lógica extra en el front.
-- La calificación ya viene precalculada desde VersionJugador
-- sin AVG ni GROUP BY en esta query.
-- Incluye EdadActual calculada desde FechaNacimiento.
--
-- Uso desde el backend:
--   SELECT * FROM vw_ListadoJugadores
--   WHERE EsActivo = TRUE AND Version = 2023
--   AND   Nombre LIKE '%texto%'   -- búsqueda por nombre
--   AND   IdClub        = ?       -- filtro por club
--   AND   IdLiga        = ?       -- filtro por liga
--   AND   IdPosicion    = ?       -- filtro por posición
--   AND   IdNacionalidad = ?      -- filtro por país
--   AND   EsHombre      = TRUE    -- solo hombres
--   AND   EsHombre      = FALSE   -- solo mujeres
--   AND   EsRetirado    = FALSE   -- solo activos en el juego
...
-- Para la pantalla de detalle individual de un jugador.
-- Devuelve TODAS las posiciones del jugador en esa versión
-- (no solo la principal) y cada skill con su valor numérico.
-- Esta vista alimenta el radar chart (Chart.js) del front
-- y la pantalla de edición del jugador.
--
-- Uso desde el backend:
--   -- Datos del jugador + todas sus skills:
--   SELECT * FROM vw_DetalleJugador
--   WHERE IdJugador = 1 AND Version = 2023;
--
--   -- Solo las posiciones del jugador en esa versión:
--   SELECT DISTINCT IdPosicion, Posicion, EsPrincipal
--   FROM vw_DetalleJugador
--   WHERE IdJugador = 1 AND Version = 2023;
--
--   -- Solo las skills para el radar chart:
--   SELECT Skill, Valor FROM vw_DetalleJugador
--   WHERE IdJugador = 1 AND Version = 2023
...
-- Solo incluye la posición principal por versión para no
-- multiplicar filas innecesariamente.
--
-- Uso desde el backend:
--   -- Historial completo (para enviar a LangChain):
--   SELECT * FROM vw_LineaDeTiempo
--   WHERE IdJugador = 1
--   ORDER BY Anio, Skill;
--
--   -- Evolución de una skill específica (para graficar):
--   SELECT Anio, Valor, Calificacion
--   FROM vw_LineaDeTiempo
--   WHERE IdJugador = 1 AND Skill = 'PAC'
--   ORDER BY Anio;
--
--   -- Todas las skills de un año específico:
--   SELECT Skill, Valor FROM vw_LineaDeTiempo
--   WHERE IdJugador = 1 AND Anio = 2023;
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 1. Listado de cards paginado (pantalla principal del front)
-- Calificacion ya viene calculada, sin AVG ni GROUP BY.
-- ------------------------------------------------------------
-- SELECT * FROM vw_ListadoJugadores
-- WHERE EsActivo   = TRUE
-- AND   Version    = 2023
-- -- Filtros opcionales (todos combinables entre sí):
-- -- AND Nombre         LIKE '%Messi%'
-- -- AND IdClub         = 1
-- -- AND IdLiga         = 1
-- -- AND IdPosicion     = 11
-- -- AND IdNacionalidad = 1
-- -- AND EsHombre       = TRUE   (solo hombres)
-- -- AND EsHombre       = FALSE  (solo mujeres)
-- -- AND EsRetirado     = FALSE  (solo activos en el juego)
-- ORDER BY Calificacion DESC
-- LIMIT 20 OFFSET 0;   -- página 1, 20 por página

-- ------------------------------------------------------------
-- 2. Detalle de un jugador con skills (para radar chart)
-- Devuelve múltiples filas: una por cada skill × posición.
-- El backend agrupa por skill para el radar chart.
-- ------------------------------------------------------------
-- SELECT * FROM vw_DetalleJugador
-- WHERE IdJugador = 1
-- AND   Version   = 2023;
...
-- ------------------------------------------------------------
-- 3. Solo las skills de un jugador (radar chart Chart.js)
-- ------------------------------------------------------------
-- SELECT DISTINCT Skill, Valor
-- FROM vw_DetalleJugador
-- WHERE IdJugador = 1 AND Version = 2023
-- ORDER BY IdSkill;

-- ------------------------------------------------------------
-- 4. Solo las posiciones de un jugador en una versión
-- ------------------------------------------------------------
-- SELECT DISTINCT IdPosicion, Posicion, EsPrincipal
-- FROM vw_DetalleJugador
-- WHERE IdJugador = 1 AND Version = 2023
...
-- ------------------------------------------------------------
-- 5. Línea de tiempo completa de un jugador
-- Se envía al LLM (LangChain) para el análisis narrativo.
-- ------------------------------------------------------------
-- SELECT * FROM vw_LineaDeTiempo
-- WHERE IdJugador = 1
-- ORDER BY Anio, Skill;
...
-- ------------------------------------------------------------
-- 6. Evolución de una skill específica a lo largo del tiempo
-- Para el gráfico de línea de tiempo en el front.
-- ------------------------------------------------------------
-- SELECT Anio, NombreVersion, Valor, Calificacion, Posicion
-- FROM vw_LineaDeTiempo
-- WHERE IdJugador = 1
-- AND   Skill     = 'PAC'
-- ORDER BY Anio;
...
-- ------------------------------------------------------------
-- 7. Filtros disponibles para los dropdowns del front
-- Se ejecutan al cargar la pantalla de listado.
-- ------------------------------------------------------------
-- SELECT DISTINCT IdLiga, Liga
-- FROM vw_FiltrosDisponibles WHERE Version = 2023
-- ORDER BY Liga;
--
-- SELECT DISTINCT IdPosicion, Posicion
-- FROM vw_FiltrosDisponibles WHERE Version = 2023
-- ORDER BY Posicion;
--
-- SELECT DISTINCT IdNacionalidad, Nacionalidad
-- FROM vw_FiltrosDisponibles WHERE Version = 2023
-- ORDER BY Nacionalidad;
--
-- SELECT DISTINCT Version, NombreVersion
-- FROM vw_FiltrosDisponibles
-- ORDER BY Version;

-- ------------------------------------------------------------
-- 8. Borrado lógico de un jugador creado por usuario
-- rowsAffected = 0 → era del juego base → error controlado
-- ------------------------------------------------------------
-- UPDATE Jugador
-- SET    EsActivo = FALSE
-- WHERE  Id             = ?
-- AND    EsDelJuegoBase = FALSE;

-- ------------------------------------------------------------
-- 9. UPDATE masivo de calificaciones post-carga del CSV
-- Se ejecuta UNA SOLA VEZ después de importar todo el CSV.
-- Mucho más eficiente que calcular fila por fila.
-- ------------------------------------------------------------
-- UPDATE VersionJugador vj
-- SET Calificacion = (
--     SELECT ROUND(AVG(Valor), 0)
--     FROM VersionJugadorSkill
--     WHERE IdVersionJugador = vj.Id
-- );

-- ------------------------------------------------------------
-- 10. Buscar usuario por proveedor OAuth (usado por Passport)
-- Se usa al recibir el callback de Google o GitHub.
-- ------------------------------------------------------------
-- SELECT * FROM Usuario
-- WHERE AuthProvider = 'google'
-- AND   ProviderId   = '104829301847263819';